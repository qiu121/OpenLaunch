...
- OpenLaunch 使用 macOS 文件系统事件监听默认扫描目录变化，不做后台定时轮询；隐藏状态下检测到变化会标记待刷新，下次打开启动台时自动重新扫描，启动台可见时会在防抖后刷新。
...
- 默认排序是添加时间从旧到新，扫描器必须优先读取 Spotlight 添加日期；用户可切换为“最近添加”倒序。
...
- 默认扫描路径会通过文件系统事件监听应用安装、删除和移动；自动刷新使用防抖，不使用定时轮询。
