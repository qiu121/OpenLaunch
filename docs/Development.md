...
- 底部仅显示分页圆点，支持点击跳页、鼠标/触控板横向滚轮翻页和横向拖拽翻页。分页轨道由 AppKit 接收高频输入并通过 Core Animation 直接位移，手势阶段不经全局可观察状态刷新；触控板主轴增量按 2 倍映射到页面轨道，在保留小幅精细移动的同时支持慢速手势停留在半页位置。释放位移达到页面跨度 15% 时进入相邻页；快速滑动还需同时满足最小移动距离和近期速度阈值，避免轻触误翻页，每次最多移动一页。
- 释放吸附由 60–120Hz 显示链路逐帧驱动，以 120Hz 每帧收敛剩余距离的 18% 为基准，并按实际帧间隔修正速度；首尾页使用最大约 10% 页面跨度的非线性橡皮筋阻力。缺少完整手势阶段的输入设备使用 140ms 空闲兜底，吸附尚未结束时新手势可以从当前画面直接接管。
- 触控板双指滚动按每个事件的主要轴向转换为分页位移，轻微斜向起步不会直接拒绝整段手势；事件缺少窗口引用时会从屏幕坐标换算到当前前台启动台窗口后再命中分页区域，系统动量开始时立即结算当前手势并抑制后续惯性跨页。
- 系统取消手势时始终回到原页；启动台隐藏后清空未完成的输入和吸附状态并复位轨道，避免重新进入时短暂显示半页。非精细鼠标滚轮按连续事件簇限制为一次翻页，显示链路仅在吸附期间运行。
